...
export const handleClicks = {
  ...
  methods: {
    handleClicks($event) {
      ...
      if (isAFile) {
        $event.preventDefault()
        console.log('isAFile:', isAFile)
        const filename = href.split('/').pop()
        console.log('Download event: ', filename)

        const path = `/irb/${filename}`
        ...
        location.href = '/irb/search.json'
        // window.open('/irb/sitemap.xml')
      }
      ...
      //       : ''
      //   const path = `${publicPath}${this.$route.meta.fileDownloadPath}${filename}`
      //   console.log('download path: ', path)
